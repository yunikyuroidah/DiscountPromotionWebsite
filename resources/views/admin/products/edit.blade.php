...
@section('title', 'Edit Barang - PromoDiskon')
@section('page-title', 'Edit Barang')
@section('page-desc', 'Perbarui informasi produk.')
...
@section('content')
    ...
    <form method="post" action="{{ route('admin.products.update', $product) }}" class="form-card" enctype="multipart/form-data">
        @csrf
        @method('put')
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input id="brand" type="text" name="brand" value="{{ old('brand', $product->brand) }}" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input id="weight_grams" type="number" name="weight_grams" value="{{ old('weight_grams', $product->weight_grams) }}" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input id="discount_percent" type="number" name="discount_percent" value="{{ old('discount_percent', $product->discount_percent) }}" min="0" max="90">
            </div>
            <div class="form-group">
                <label for="discount_valid_until">Masa Berlaku Diskon</label>
                <input id="discount_valid_until" type="date" name="discount_valid_until" value="{{ old('discount_valid_until', $product->discount_valid_until?->format('Y-m-d')) }}">
            </div>
            <div class="form-group form-group-full">
                <label>Gambar Produk</label>
                <div class="dropzone {{ $product->image ? 'has-preview' : '' }}" id="dropzone">
                    <input id="image" type="file" name="image" accept="image/*" class="dropzone-input">
                    <div class="dropzone-body" id="dropzone-body" style="{{ $product->image ? 'display:none' : '' }}">
                    ...
                    <div class="dropzone-preview-wrap" id="dropzone-preview" style="{{ $product->image ? 'display:flex' : 'display:none' }}">
                        <img class="dropzone-preview-img" id="dropzone-preview-img"
                             src="{{ $product->image ? route('product.image', $product) : '' }}"
                             alt="Preview">
                        <button type="button" class="dropzone-remove" id="dropzone-remove" title="Ganti gambar">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                        </button>
                        <p class="dropzone-filename" id="dropzone-filename">{{ $product->image ? 'Gambar tersimpan di database' : '' }}</p>
                    </div>
                </div>
            </div>
            <div class="form-group form-group-full">
